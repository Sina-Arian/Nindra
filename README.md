...
📦 Installation
    ...
    Copy the addons/nindra/ folder into your project’s addons/ directory.

    Enable the plugin in Project → Project Settings → Plugins.

    A new dock “Nindra” will appear on the right side.

    The plugin assumes Ollama is running on http://127.0.0.1:11434. If you changed the port, edit OLLAMA_CHAT_URL and EMBED_URL in chat_dock.gd.
...
A simple exporter is **not** included – you can write a small tool or use the https://github.com/1Stalk/godot-context-exporter addon.  
...
🐛 Known limitations
    ...
    The chunker uses regex – it may fail on very complex or malformed code.

    Only context_export.txt is supported; dynamic live indexing is not implemented.

    Embedding requests are blocking for the UI while indexing (a send_button.disabled is set, but the editor may feel sluggish for large projects).

    Streaming uses HTTPClient.poll() in _process – be careful not to cause performance issues with many open docks.

    The plugin assumes Ollama is always reachable; missing models or connection errors will show red messages.
